...
import { ChevronRight, ShoppingCart, Check, Brain, FileText, Phone, CheckCircle } from "lucide-react";
...
const PLANS = [
  {
    ...
    unit: "per profile",
    icon: FileText,
    description: "AI resume evaluation against your custom rubric — ranked shortlist in seconds.",
    features: ["Resume parsing & deduplication", "Rubric-based scoring", "Shortlist export"],
  },
  {
    id: "screening-call",
    name: "Screening Call",
    ...
    unit: "per call (5 min)",
    icon: Phone,
    description: "Automated 5-min AI screening call with transcript and sentiment analysis.",
    features: ["AI-conducted 5-min call", "NLP sentiment analysis", "Full transcript"],
    isRecommended: true,
  },
  {
    id: "ai-interview",
    ...
    unit: "per interview",
    icon: Brain,
    description: "Full AI voice interview with adaptive questions, video recording, and 10-parameter report.",
    features: ["Adaptive question flow", "Video + transcript", "10-parameter report"],
  },
];
...
export default function DecoTalentConfigurePage() {
  const router = useRouter();
  const { addItem } = useCartStore();

  const [selections, setSelections] = useState<Selection[]>([{ planId: "screening-call", quantity: 1 }]);
  ...
  const togglePlan = (planId: string) => {
    setSelections((prev) => {
      ...
      if (exists) {
        return prev.filter((s) => s.planId !== planId);
      }
      return [...prev, { planId, quantity: 1 }];
    });
  };

  const updateQty = (planId: string, delta: number) => {
    setSelections((prev) =>
      prev.map((s) =>
        s.planId === planId ? { ...s, quantity: Math.max(1, s.quantity + delta) } : s
      )
    );
  };

  const isSelected = (planId: string) => selections.some((s) => s.planId === planId);
  const getQty = (planId: string) => selections.find((s) => s.planId === planId)?.quantity ?? 1;

  const getPlanPrice = (plan: typeof PLANS[0]) => plan.price * cycle.multiplier;

  const total = selections.reduce((sum, s) => {
    const plan = PLANS.find((p) => p.id === s.planId);
    return sum + (plan ? getPlanPrice(plan) * s.quantity : 0);
  }, 0);
  ...
  const handleAddToCart = () => {
    selections.forEach((s) => {
      ...
      if (!plan) return;
      addItem({
        product: { id: "deco-talent", slug: "deco-talent", name: "Deco Talent" } as any,
        quantity: s.quantity,
        selectedAddons: [],
        billingCycle: billingCycle.toUpperCase().replace("-", "_") as any,
        isRecurring: true,
        unitPrice: getPlanPrice(plan),
        totalPrice: getPlanPrice(plan) * s.quantity,
        recurringAmount: getPlanPrice(plan),
        variantId: s.planId,
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-5xl mx-auto px-4 md:px-6 lg:px-8 py-4">
          <nav className="flex items-center justify-center gap-2 text-sm text-gray-400">
            ...
            <ChevronRight className="h-3.5 w-3.5" />
            <Link href="/products/deco-talent" className="hover:text-[#1E2260] transition-colors">Deco Talent</Link>
            <ChevronRight className="h-3.5 w-3.5" />
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#EEF2FF] text-[#1E2260] text-xs font-semibold mb-3">
            <Brain className="w-3.5 h-3.5" /> AI Interview Bot · Pre-Paid
          </div>
          <h1 className="text-3xl font-bold text-gray-900">Configure Your Plan</h1>
          <p className="mt-2 text-gray-500 text-sm max-w-md mx-auto">
            Select one or more services. Pay per use — no seat limits, 1 year validity.
          </p>
        </div>
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="text-right shrink-0 ml-2">
                        <p className="text-xl font-black text-[#1E2260]">₹{getPlanPrice(plan).toLocaleString("en-IN")}</p>
                        <p className="text-[10px] text-gray-400">{plan.unit}{cycle.multiplier > 1 ? ` × ${cycle.multiplier}` : ""}</p>
                      </div>
                    </div>
                  </button>

                  {/* Quantity row — shown only when selected */}
                  {selected && (
                    <div className="px-5 pb-4 flex items-center justify-between border-t border-[#1E2260]/10 pt-3">
                      ...
                      <div className="flex items-center gap-2 shrink-0 ml-4">
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); updateQty(plan.id, -1); }}
                          disabled={qty <= 1}
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-5">
                    ...
                    return (
                      <div key={s.planId} className="flex justify-between text-sm">
                        <span className="text-gray-600">
                          {plan.name}
                          {s.quantity > 1 && <span className="text-gray-400 ml-1">(×{s.quantity})</span>}
                        </span>
                        <span className="font-medium text-gray-900">
                          ₹{(getPlanPrice(plan) * s.quantity).toLocaleString("en-IN")}
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 space-y-1.5">
                {["1 Year Validity", "Unlimited Job Postings", "Unlimited HR Seats", "No monthly commitment"].map((item) => (
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-5">
              ...
              <ul className="space-y-2">
                {["1 Year Validity", "Unlimited Job Postings", "Unlimited Candidate Data", "Unlimited Seats / HR Users", "Taxes as applicable"].map((item) => (
                  <li key={item} className="flex items-center gap-2 text-sm text-gray-700">
