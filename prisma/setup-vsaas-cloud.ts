const { PrismaClient, AddonPricingType } = require('@prisma/client')

const prisma = new PrismaClient()

async function main() {
  console.log('Setting up VSAAS Cloud with INR pricing (Customer Rates)...')

  // First, find or create the VSAAS category
  ...
  // Find existing VSAAS Cloud product
  let product = await prisma.product.findFirst({
    where: { 
      name: { contains: 'VSAAS Cloud', mode: 'insensitive' }
    }
  })

  if (!product) {
    // Create VSAAS Cloud product
    ...
    console.log('Created VSAAS Cloud product:', product.id)
  } else {
  ...
  // Store pricing in attributes JSON
  ...
  console.log('\n=== VSAAS Cloud Setup Complete (INR Pricing) ===')
